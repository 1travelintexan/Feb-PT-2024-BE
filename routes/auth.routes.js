...
router.post("/signup", uploader.single("imageUrl"), async (req, res, next) => {
  ...
  try {
    ...
    // Create the salt and encrypt the password asynchronously -  - genSalt() hash()
...
//GET route to get all the info for the current profile on the frontend
router.get("/profile/:userId", async (req, res) => {
  try {
    const currentUser = await User.findById(req.params.userId).lean();
    delete currentUser.password;
    console.log("here is the current user for the profile page", currentUser);
